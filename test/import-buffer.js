...
function getImportBuffer(options) {
    ...
    const importBuffer = proxyquire("../release/import-buffer", {
        fs: fsStub,
        os: osStub,
        mkdirp: mkdirpStub
    });
    ...
    describe("when creating an instance", () => {
        ...
        [
            ...
            it(`should throw if created with ${item.name} importer`, () => {
                expect(() => new ImportBuffer(item.value, "xxx")).to.throw(
                    "Invalid importer."
                );
            });
        });
        ...
        it("should throw is created without a buffer key", () => {
            expect(() => new ImportBuffer(async () => [])).to.throw(
                "A buffer key is required."
            );
        });
    });
    ...
    describe("when there are existing imports", () => {
        ...
        it("should provide stats for existing imports", async () => {
            ...
            const transformedImports = imports
                .map(i => {
                    return { path: i.path, time: i.time };
                })
                .sort();
            expect(transformedImports).to.deep.equal([
                { path: "import1.less", time: date1.getTime() },
        ...
        it("should serialise list of imports to disk", async () => {
            ...
            const mainFileDir = crypto
                .createHash("md5")
                .update(mainFile.path)
                .digest("hex");
            const serialiseDir = path.join(tempDir, bufferKey);
            const tempFilePath = path.join(
                serialiseDir,
                mainFileDir + "_" + path.basename(mainFile.path)
            );

            const imports = await buffer.listImports(mainFile);
            expect(mkdirpStub).to.have.been.calledWith(
                serialiseDir,
                sinon.match.func
            );
        ...
        it("should log error to console if serialised file cannot be written", async () => {
            ...
            expect(console.error).to.have.been.calledWith(
                `Failed to cache results to '${tempFilePath}'. Error: Something went wrong.`
            );
        });
        ...
        it("should call original importer again if modified times have not changed but using instance with different buffer key", async () => {
            ...
            const buffer2 = new ImportBuffer(
                fakeImportLister.listImports.bind(fakeImportLister),
                "some different key"
            );
            ...
            const transformedImports = imports
                ...
                .sort();
            expect(transformedImports).to.deep.equal([
                { path: "import3.less", time: date1.getTime() },
        ...
        it("should not call original importer if serialised modified times have not changed", async () => {
            const serialisedImports = [
                { path: "import1.less", time: date1.getTime() },
                { path: "import2.less", time: date2.getTime() }
            ];
            spyContext
                .stub(fsStub, "readFile")
                .callsFake((_, done) =>
                    done(null, JSON.stringify(serialisedImports))
                );
        ...
        it("should log error to console if serialised file cannot be read", async () => {
            ...
            await buffer.listImports(mainFile);
            expect(console.error).to.have.been.calledWith(
                `Failed to load cached results from '${tempFilePath}'. Error: Something went wrong.`
            );
        });
