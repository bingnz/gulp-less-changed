...
describe('path-resolver', () => {
    ...
    describe('when paths are not specified', () => {
        ...
        it('should also try current directory', async () => {
            ...
            expect(resolved).to.equal(path.normalize(path.join(currentDirectory, filePath)));
        });
    });
    ...
    describe('when paths are specified', () => {
        it('should return path if file exists in one of the paths', async () => {
            ...
            const resolved = await pathResolver.resolve('./', fileName, [anotherPath, thisPath, yetAnotherPath]);
        ...
        it('should throw error if file doesn\'t exist in any of the paths', async () => {
            ...
            try {
                await pathResolver.resolve(currentDirectory, fileName, [anotherPath, yetAnotherPath]);
                expect.fail(1, 0, 'Should have thrown an error.');
            } catch (error) {
        ...
        it('should also try current directory', async () => {
            ...
            fakeFs.file(path.join(currentDirectory, filePath), { stat: { mtime: new Date() } });
            const resolved = await pathResolver.resolve('./', filePath, ['bad1', 'bad2']);
